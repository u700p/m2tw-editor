...
import { Upload, Download, Plus, Trash2, AlertTriangle, Shield, X, Copy, MoveUp, MoveDown } from 'lucide-react';
...
`;
  ...
  const lines = (f) => {
    ...
    f.tertiary_colour && (f.tertiary_colour.r !== 0 || f.tertiary_colour.g !== 0 || f.tertiary_colour.b !== 0) ? `tertiary_colour${T3}${fmtC(f.tertiary_colour)}` : null,
      ...
      <div className="p-4 space-y-4 max-w-2xl">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-4 bg-slate-700">
            <TabsTrigger value="identity" className="text-[10px]">Identity</TabsTrigger>
            <TabsTrigger value="colours" className="text-[10px]">Colours</TabsTrigger>
            <TabsTrigger value="files" className="text-[10px]">Files</TabsTrigger>
            <TabsTrigger value="horde" className="text-[10px]">Horde</TabsTrigger>
          </TabsList>

          <TabsContent value="identity" className="space-y-2 mt-3">
            <h3 className="text-[10px] uppercase tracking-wider text-slate-400 font-semibold border-b border-slate-600 pb-1">Basic Info</h3>
            ...
            <h3 className="text-[10px] uppercase tracking-wider text-slate-400 font-semibold border-b border-slate-600 pb-1 pt-3">Flags</h3>
          ...
          <TabsContent value="colours" className="space-y-2 mt-3">
            <h3 className="text-[10px] uppercase tracking-wider text-slate-400 font-semibold border-b border-slate-600 pb-1">Faction Colours</h3>
            ...
            <ColourPickerField label="Tertiary Colour" colour={draft.tertiary_colour} onChange={(v) => set('tertiary_colour', v)} />
          ...
          <TabsContent value="horde" className="space-y-2 mt-3">
            <h3 className="text-[10px] uppercase tracking-wider text-slate-400 font-semibold border-b border-slate-600 pb-1">Horde Settings</h3>
            <div className="flex items-center justify-between py-0.5">
...
export default function FactionsEditor() {
  ...
  const [dupTargetIdx, setDupTargetIdx] = useState(null);
  ...
  const confirmDuplicate = () => {
    if (!dupTargetIdx || !dupName.trim()) return;
    const src = factions[dupTargetIdx];
    const nameUpper = dupName.trim().toUpperCase();
    const dup = {
      ...src,
      name: dupName.trim(),
      spawn_type: 'default',
      shadow_faction: '',
      symbol: `models_strat/symbol_${dupName.trim()}.CAS`,
      loading_logo: `loading_screen/symbols/symbol128_${dupName.trim()}.tga`,
  ...
  const moveFaction = (i, direction) => {
    const newIdx = i + direction;
    if (newIdx < 0 || newIdx >= factions.length) return;
          ...
          <div className="w-56 border-r border-border flex flex-col shrink-0">
            ...
            <div key={i} className={`w-full flex items-center gap-2 px-3 py-2 border-b border-border/60 ${selectedIdx === i ? 'bg-accent' : 'hover:bg-accent'}`}>
                  ...
                  <div className="flex gap-0.5 shrink-0 items-center">
                    <button onClick={(e) => {e.stopPropagation();moveFaction(i, -1);}}
                className="text-slate-400 hover:text-slate-200 p-1" title="Move Up" disabled={i === 0}>
                      <MoveUp className="w-3 h-3" />
                    </button>
                    <button onClick={(e) => {e.stopPropagation();moveFaction(i, 1);}}
                className="text-slate-400 hover:text-slate-200 p-1" title="Move Down" disabled={i === factions.length - 1}>
                      <MoveDown className="w-3 h-3" />
                    </button>
                    <button onClick={(e) => {e.stopPropagation();openDuplicate(i);}}
        ...
        <Dialog open={dupDialogOpen} onOpenChange={setDupDialogOpen}>
          <DialogContent className="bg-slate-800 border-slate-600">
            ...
            <div className="space-y-3 py-3">
              <p className="text-sm text-slate-300">Enter a name for the new faction:</p>
              <Input 
                value={dupName} 
              ...
              />
              <p className="text-[10px] text-slate-400">Symbol, logo indices, and file paths will be auto-generated based on this name.</p>
            </div>
            <DialogFooter>
              <button onClick={() => setDupDialogOpen(false)} className="px-4 py-2 text-[11px] rounded border border-slate-600 text-slate-300 hover:bg-slate-700">Cancel</button>
              <button onClick={confirmDuplicate} className="px-4 py-2 text-[11px] rounded bg-blue-700 hover:bg-blue-600 text-white font-semibold">Duplicate</button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      )}
